    ...
    <AppShell>
      <div style={{ padding: '32px 32px 48px', maxWidth: 1280, margin: '0 auto' }}>
        <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start' }}>
          {/* Sidebar */}
          <Sidebar mode="org" onNewClick={handleNewProject} />

          {/* Main area */}
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 20 }}>
            <AnimatePresence mode="wait">
              {!loaded ? (
                <EmptyWorkbench key="empty" mode="org" onLoaded={handleLoaded} />
              ) : (
                <motion.div
                  key="loaded"
                  initial={{ opacity: 0 }}
                ...
                >
                  <ProjectHeader mode="org" analysesRun={analysesRun} />
                  <DocumentTable documents={documents.org} onPreview={setPreviewDoc} />
